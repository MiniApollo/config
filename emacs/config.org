...
#+begin_src emacs-lisp
    (use-package evil
      ...
      (setq evil-undo-system 'undo-tree) ;; ctrl r to work (redo) with UndoTree for stock Emacs use 'undo-redo
...
#+begin_src emacs-lisp
    (use-package general
      ...
      (mark/leader-keys
        "h" '(:ignore t :wk "Help")
        "h f" '(flycheck-list-errors :wk "Flycheck list errors buffer")
...
** Flycheck 
Flycheck is recommended by lsp-mode for better error handling
#+begin_src emacs-lisp
...
#+end_src
* Runtime Performance
Dial the GC threshold back down so that garbage collection happens more frequently but in less time and increase Read Process Output Max so emacs can read more data
#+begin_src emacs-lisp
